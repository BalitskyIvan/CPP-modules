...
int main() {
	...
	for (int i = 0; i < randomNum; i++)
		ninjaTrap[i] = new NinjaTrap(names_ninja[i]);
	while (isAlive(fragTrap, scavTrap, randomNum)) {
		...
		for (int i = 0; i < randomNum; i++) {
			...
			if (fragTrap[attackOnRobot]->getHitPoints() != 0 && scavTrap[i]->getHitPoints() != 0) {
				...
				if (randomAttack == 0)
					fragTrap[attackOnRobot]->takeDamage(ninjaTrap[i]->ninjaShoebox(*fragTrap[i]));
				else
					scavTrap[attackOnRobot]->takeDamage(ninjaTrap[i]->ninjaShoebox(*scavTrap[i]));
			}
		}
	}
	for (int i = 0; i < randomNum; i++)
